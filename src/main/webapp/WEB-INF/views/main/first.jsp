			...
			$(document).ready(function () {
				...
				parentModal.on("click", "#checkPassword", function () {
					...
					if (pwd == "" || pwd == null) {
						...
						// 상태가 열람가능일 경우 비밀번호가 유효한지 확인하고 second 페이지로 이동한다.
						if (text == "열람가능") {
							$.ajax({
								type: "get",
								url: "/pwdCheck",
						...
						// 상태가 수정중이거나 열람불가일 경우 비밀번호가 유효한지 확인하고 상태를 열람가능으로 변경한다.
						else if (text == "수정중" || text == "열람불가") {
							$.ajax({
								type: "get",
								url: "/pwdCheckWChangePassword",
								dataType: "json",
								contentType: "application/json; charset=utf-8",
								data: { "pwd": pwd, "cnt": cnt },
								success(result) {
													...
													<td>
														...
														<c:choose>
															<c:when test="${dataList.status eq '열람가능'}">
																<span class="badge bg-success" data-toggle="modal" data-target="#pwdModal"
																	class="accessible">열람가능</span>
															</c:when>
															<c:when test="${dataList.status eq '수정중'}">
																<span class="badge bg-warning" data-toggle="modal" data-target="#pwdModal"
																	class="modifying">수정중</span>
															</c:when>
															<c:when test="${dataList.status eq '열람불가'}">
																<span class="badge bg-danger" data-toggle="modal" data-target="#pwdModal"
																	class="inaccessible">열람불가</span>
															</c:when>
														</c:choose>
													</td>
												</tr>
